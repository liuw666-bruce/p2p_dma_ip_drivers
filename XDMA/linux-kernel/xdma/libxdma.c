...
static void xdma_phy_mem_request_cb_dump(struct xdma_request_cb *req)
{
	int i;

	pr_info("request 0x%p, total %u, ep 0x%llx, sw_desc %u.\n",
		req, req->total_len, req->ep_addr, req->sw_desc_cnt);
...
static struct xdma_request_cb *xdma_phy_addr_init_request(u64 phy_addr,
						 u64 ep_addr, u64 len)
{
	struct xdma_request_cb *req;
	int max = 1;
	...
	req->sw_desc_cnt = j;
#ifdef __LIBXDMA_DEBUG__
	xdma_phy_mem_request_cb_dump(req);
#endif
	return req;
}
...
ssize_t xdma_phy_addr_xfer_submit(void *dev_hndl, int channel, bool write, u64 ep_addr,
			 u64 phy_addr, u64 len, int timeout_ms)
{
	struct xdma_dev *xdev = (struct xdma_dev *)dev_hndl;
	struct xdma_engine *engine;
	int rv = 0, tfer_idx = 0, i;
	...
	req = xdma_phy_addr_init_request(phy_addr, ep_addr, len);
	if (!req) {
		rv = -ENOMEM;
		goto unmap_sgl;
	}
