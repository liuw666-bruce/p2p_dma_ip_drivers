...
static int ioctl_do_phy_mem_dma(struct xdma_engine *engine, unsigned long arg)
{
	int rv;
	ssize_t res;
	struct xdma_dev *xdev;
	...
	unsigned long len;
	bool write;
	struct xdma_phy_addr_ioctl xdma_phy_addr;
	...
	rv = copy_from_user(&xdma_phy_addr,
		(struct xdma_phy_addr_ioctl __user *)arg,
		sizeof(struct xdma_phy_addr_ioctl));
	...
	ep_addr = xdma_phy_addr.ep_addr;
	phy_addr = xdma_phy_addr.phy_addr;
	direction = xdma_phy_addr.direction;
	len = xdma_phy_addr.len;
	write = (direction == DIRECTION_FROM_PHY_ADDR);

	res = xdma_phy_addr_xfer_submit(xdev, engine->channel, write, ep_addr, phy_addr, len,
				write ? h2c_timeout * 1000 :
					   c2h_timeout * 1000);
		...
		unsigned long arg)
{
	...
	case IOCTL_XDMA_APERTURE_W:
		...
		break;
	case IOCTL_XDMA_PHY_ADDR:
		rv = ioctl_do_phy_mem_dma(engine, arg);
		break;
	default:
