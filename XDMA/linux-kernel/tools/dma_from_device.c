...
static int test_dma(char *devname, uint64_t addr, uint64_t aperture,
			...
			char *ofname, bool access_phy_address, uint64_t phy_address);
...
int main(int argc, char *argv[])
{
	...
	bool access_phy_address = false;
	char *ofname = NULL;

	while ((cmd_opt = getopt_long(argc, argv, "vhec:f:d:a:p:k:s:o:", long_opts,
			    NULL)) != -1) {
		switch (cmd_opt) {
		case 0:
			/* long option */
		...
		case 'a':
			...
			break;
		case 'p':
			/* Physical address on the AXI_LITE bus in bytes */
			phy_address = getopt_integer(optarg);
			access_phy_address = true;
			break;
		case 'k':
	...
	if (verbose)
	fprintf(stdout,
		"dev %s, addr 0x%lx, phy_address 0x%lx, aperture 0x%lx, size 0x%lx, offset 0x%lx, "
		"count %lu\n",
		device, address, phy_address, aperture, size, offset, count);

	return test_dma(device, address, aperture, size, offset, count, ofname, access_phy_address, phy_address);
}
	...
	for (i = 0; i < count; i++) {
		...
		if (aperture && (buffer != NULL)) {
			...
			if (rc < 0)
				goto out;
			bytes_done = rc;
		} else if (access_phy_address) {
			rc = read_to_phy_mem(devname, fpga_fd, phy_address, size, addr);
			if (rc < 0)
				goto out;
			bytes_done = rc;
		}
