...
ssize_t read_to_phy_mem(char *fname, int fd, uint64_t phy_addr, uint64_t size,
			uint64_t base)
{
	ssize_t rc;
	uint64_t count = 0;	
	...
	while (count < size) {
		...
		rc = ioctl(fd, IOCTL_XDMA_PHY_ADDR, &io);
		if (rc < 0) {
 			fprintf(stderr, "ioctl failed %ld.\n", rc);
			perror("read to phy addr");
			return -EIO;
		}

		count += rc;
		if (rc != bytes) {
			fprintf(stderr, "%s, read to phy mem underflow 0x%lx/0x%lx @ 0x%lx->0x%lx.\n",
				fname, rc, bytes, base, phy_addr);
			break;
		}
	...
	if (count != size && loop)
		fprintf(stderr, "%s, read to phy mem underflow 0x%lx/0x%lx.\n",
			fname, count, size);
	return count;
}
...
ssize_t write_from_phy_addr(char *fname, int fd, uint64_t phy_addr, uint64_t size,
			uint64_t base)
{
	ssize_t rc;
	uint64_t count = 0;
	...
	while (count < size) {
		...
		if (rc < 0) {
 			fprintf(stderr, "ioctl failed %ld.\n", rc);
			perror("write from phy addr");
			return -EIO;
		}

		count += rc;
		if (rc != bytes) {
			fprintf(stderr, "%s, write from phy addr underflow 0x%lx/0x%lx @ 0x%lx->0x%lx.\n",
				fname, rc, bytes, phy_addr, base);
			break;
		}
		phy_addr += bytes;
		base += bytes;
	...
	if (count != size && loop)
		fprintf(stderr, "%s, write from phy addr underflow 0x%lx/0x%lx.\n",
			fname, count, size);
