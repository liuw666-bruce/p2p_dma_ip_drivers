...
static int test_dma(char *devname, uint64_t addr, uint64_t aperture,
		    ...
		    char *infname, char *ofname, bool access_phy_address, uint64_t phy_address);
...
int main(int argc, char *argv[])
{
	...
	while ((cmd_opt =
		getopt_long(argc, argv, "vhc:f:d:a:p:k:s:o:w:", long_opts,
			    NULL)) != -1) {
		switch (cmd_opt) {
		case 0:
			/* long option */
	...
	return test_dma(device, address, aperture, size, offset, count,
			infname, ofname, access_phy_address, phy_address);
}
		    ...
		    char *infname, char *ofname, bool access_phy_address, uint64_t phy_address)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (aperture && (buffer != NULL)) {
			...
			rc = write_from_phy_addr(devname, fpga_fd, phy_address, size,
				      	 	addr);
			if (rc < 0)
				goto out;
