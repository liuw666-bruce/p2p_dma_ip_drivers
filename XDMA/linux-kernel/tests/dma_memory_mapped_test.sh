...
echo -e "\ttransfer size:  $transferSz, count: $transferCount, phyAddr: $phyAddr"
...
if [ $h2cChannels -gt 0 ]; then
	...
	for ((i=0; i<=3; i++)); do
		addrOffset=$(($dmaAddrbase+$(($transferSz * $i))))
...
# Read from all enabled c2hChannels in parallel to physical addr (device)
if [ $phyAddr -ne 0 ]; then
	if [ $c2hChannels -gt 0 ]; then
		...
		for ((i=0; i<=3; i++)); do
			...
			echo "Info: Reading from c2h channel $curChannel at " \
				"address offset $addrOffset to phy addr $(($phyAddr+$addrOffset))."
			$tool_path/dma_from_device -d /dev/${xid}_c2h_${curChannel} \
		       	-p $(($phyAddr+$addrOffset)) -s $transferSz \
		       	-a $addrOffset -c $transferCount &
			# If all channels have active transactions we must wait for
				# them to complete
...
# Wait for the last transaction to complete.
	wait
# move to another dma addr
	dmaAddrbase=$(($addrOffset+$transferSz))
fi



# Write to all enabled h2cChannels in parallel from physical addr (device)
if [ $phyAddr -ne 0 ]; then
	if [ $h2cChannels -gt 0 ]; then
		# Loop over four blocks of size $transferSz and write to them
		for ((i=0; i<=3; i++)); do
			addrOffset=$(($transferSz * $i))
			DMAaddrOffset=$(($dmaAddrbase+$(($transferSz * $i))))
			curChannel=$(($i % $h2cChannels))
				echo "Info: Writing to h2c channel $curChannel at address" \
				   "offset $DMAaddrOffset from phy addr  $(($phyAddr+$addrOffset))."
			$tool_path/dma_to_device -d /dev/${xid}_h2c_${curChannel} \
					-p $(($phyAddr+$addrOffset)) -s $transferSz \
				-a $DMAaddrOffset -c $transferCount &
			# If all channels have active transactions we must wait for
				# them to complete
			if [ $(($curChannel+1)) -eq $h2cChannels ]; then
				echo "Info: Wait for current transactions to complete."
